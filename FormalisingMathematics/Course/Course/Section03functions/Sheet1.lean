...
example (f : X → Y) (g : Y → Z) : Injective (g ∘ f) → Injective f := by
  ...
  rw [injective_def] at *
  intro a b f_eq
  specialize inj_comp a b
  have hG : g (f a) = g (f b) := by exact congrArg g f_eq
  apply inj_comp at hG
  exact hG
...
example (f : X → Y) (g : Y → Z) : Surjective (g ∘ f) → Surjective g := by
  ...
  intro b
  specialize sur_comp b
  cases' sur_comp with a surj
  use f a
  apply surj
